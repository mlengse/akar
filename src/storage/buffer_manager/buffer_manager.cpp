...
void BufferManager::updateFrameIfPageIsInFrame(file_idx_t fileIdx, const uint8_t* newPage,
    ...
    // and concurrent lock/unlock (which change the version). We cannot use spinLock(old)
    // because that retries with a stale expected value and can spin forever if the version
    // has changed.
    while (true) {
