...
public:
    ...
    // Thread-safe variant of updateFrameIfPageIsInFrameWithoutLock that acquires the page state
    // lock before updating the frame. The version increment on unlock ensures that concurrent
    // optimistic readers will detect the change and retry their read.
