...
commit already succeeded — a broken/absent index only degrades searchability, `tracing::warn!`).
`doc_id` is now `INDEXED | STORED` (both fields etched in the index schema), which makes
`delete_term` delete the exact previous document. **Caveat:** on-disk indexes built before
P107.1 have `doc_id` as FAST-only, so `delete_term` is a silent no-op for them — UPDATE/DELETE
stale those indexes until a `DROP`/`CREATE FTS INDEX` rebuild.

**Crash recovery (P107.3):** the Tantivy segment commit is the crash boundary —
`IndexWriter::commit()` persists pending segments (fsync + `meta.json` generation swap); a crash
before it loses only the in-flight increment, never the committed state, and never over-commits
(uncommitted ops are unsearchable). A crash between the durable akar commit and the hook's Tantivy
commit leaves the index **stale** (missing that increment) but uncorrupted — the non-fatal hook
contract degrades to lower searchability, which is exactly P107.3's "last committed state aman".
Recovery is a reopen: a fresh catalog (empty FTS registry) lazily reopens `<db_path>/fts/<idx>` via
`akar_fts::index::runtime_handle`, and the commit-time sync keeps working on the reopened writer
path. Pinned by `test_fts_crash_recovery_last_committed_survives` (akar-fts — a writer dropped
without `commit()` is the crash: reopen sees only the last committed state and the index stays
writable) and `test_fts_crash_recovery_across_db_reopen` (akar-main — DB dropped and recreated on
the same directory simulates crash + restart: committed rows searchable, post-restart DML syncs
into the same reopened index).

**Transaction boundary (P107.4):** FTS visibility is **commit-gated** — the propagation + reader
reload run only on a *successful durable commit* (Step 4.5 in `commit_write_txn`, after
`commit_transaction` succeeded). A write statement in an explicit `BEGIN`/`COMMIT`/`ROLLBACK`
transaction is searchable to a search in that same transaction once its statement commits, and
stays searchable after `COMMIT`; an **aborted** write (statement error → connection wrapper rolls
the transaction back) is *never* searchable — not inside the transaction, not after `ROLLBACK` —
because the sync hook never ran for it. Committed rows stay searchable throughout, so an aborted
write never desynchronises the on-disk index. Pinned by
`test_fts_insert_search_same_transaction_visible_after_commit` (akar-main). This is the
transaction-consistency half of the P107.3 crash contract: a write that never becomes a durable
commit must never leak into the Tantivy index.
...
- **Current version:** `0.1.13` (workspace); latest release tag **v0.1.13** (2026-08-26)
...
5. **Vector ANN read path is active from SQL** — the optimizer pass `VectorSimilarityDetection` (P71.4)
   rewrites `MATCH ... WHERE cosine_similarity(n.col, q) >/= thr ORDER BY cos DESC LIMIT k` into a `VectorSimilarityScan`
   (HNSW) with threshold/projection/order preserved; `CALL vector_similarity_scan(...)` (P71.1–P71.3) is the explicit
   path. The HNSW index is still maintained (refreshed after DML, P52.38).
