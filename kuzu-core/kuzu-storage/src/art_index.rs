...
//! byte encoding of a Kuzu Value), and each leaf stores row offsets (`u64`).
//!
...
mod tests {
    ...
    fn make_key(v: i64) -> ArtKey {
        ArtKey::from_value(&kuzu_common::types::Value::Int64(v)).unwrap()
    }

    fn make_str_key(s: &str) -> ArtKey {
        ArtKey::from_value(&kuzu_common::types::Value::String(s.into())).unwrap()
    }
