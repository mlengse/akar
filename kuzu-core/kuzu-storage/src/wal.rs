...
impl WAL {
    pub fn new(path: PathBuf) -> Self {
        Self { path, records: Vec::new(), total_size: 0, is_dirty: false }
    }
    ...
    pub fn clear(&mut self) { self.records.clear(); self.total_size = 0; self.is_dirty = false; }
    ...
    pub fn flush_to_disk(&self) -> std::io::Result<()> {
        ...
        for record in &self.records {
            match record {
                ...
                WALRecord::Update { table_id, row_id, column, data } => {
                    file.write_all(b"U")?;
                    table_id.serialize(&mut file)?;
                    row_id.serialize(&mut file)?;
                ...
                WALRecord::Checkpoint => { file.write_all(b"K")?; }
            }
        }
        file.flush()?;
        Ok(())
...
mod tests {
    ...
    fn test_wal_replay() {
        let dir = tempfile::tempdir().unwrap();
        let mut wal = WAL::new(dir.path().join("wal.log"));
        wal.append(WALRecord::Insert { table_id: 1, data: vec![10, 20] });
