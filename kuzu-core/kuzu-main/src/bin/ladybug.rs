...
fn main() {
    ...
    if filter == "all" || filter.contains("group") {
        ...
        run_bench!("group_by_active", "MATCH (p:Person) RETURN p.active, COUNT(p), AVG(p.score)");
        println!();
    }
    ...
    if filter == "all" || filter.contains("sort") {
        ...
        run_bench!("sort_multi", "MATCH (p:Person) RETURN p.ID, p.name ORDER BY p.age, p.score");
        run_bench!("sort_top10", "MATCH (p:Person) RETURN p.ID ORDER BY p.score DESC LIMIT 10");
        println!();
    }

    if filter == "all" || filter.contains("join") {
        println!("--- Joins ---");
        run_bench!("knows_join", "MATCH (a:Person)-[:Knows]->(b:Person) RETURN a.ID, b.ID LIMIT 1000");
        run_bench!("city_join", "MATCH (p:Person)-[:LivesIn]->(c:City) RETURN p.name, c.name LIMIT 1000");
        run_bench!("multi_hop", "MATCH (a:Person)-[:Knows]->(b:Person)-[:LivesIn]->(c:City) RETURN a.ID, c.name LIMIT 500");
        println!();
    }

    if filter == "all" || filter.contains("complex") {
        println!("--- Complex Queries ---");
        run_bench!("filter_sort_limit", "MATCH (p:Person) WHERE p.age > 25 RETURN p.ID, p.name ORDER BY p.score DESC LIMIT 20");
        run_bench!("filter_agg_complex", "MATCH (p:Person) WHERE p.active = true RETURN COUNT(p), AVG(p.age), SUM(p.score)");
        println!();
    }

    println!("==========================");
    println!("  Total benchmarks: {}", count);
    println!("  Average per query: {:.2?}", if count > 0 { total / count } else { std::time::Duration::ZERO });
    println!();
...
fn setup_schema(conn: &Connection) {
    ...
    conn.query("CREATE REL TABLE LivesIn(FROM Person TO City, since INT64)").unwrap();
}
...
fn load_data(conn: &Connection) {
    ...
    for i in 0..5000u64 {
        ...
        conn.query(&format!("MATCH (a:Person {{ID: {i}}}), (b:Person {{ID: {target}}}) CREATE (a)-[:Knows {{since: 2020}}]->(b)")).unwrap();
    }
    for i in 0..10_000u64 {
        let city_idx = i % 10;
