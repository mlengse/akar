...
fn test_create_and_query_fts_index() -> Result<(), String> {
    ...
    conn.query("CREATE (d:Document {id: 2, title: 'Rust Language', content: 'A systems programming language using Rust'})")?;
    ...
    assert!(terms_res.chunks.first().unwrap().size > 0, "FTS terms table should be populated");
