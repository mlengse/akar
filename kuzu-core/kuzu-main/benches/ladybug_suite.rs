...
fn bench_filter(c: &mut Criterion) {
    ...
    group.bench_function("filter_active_and_young", |b| {
        b.iter(|| {
            let r = conn.query(black_box("MATCH (p:Person) WHERE p.active = true AND p.age < 30 RETURN p.ID"));
            black_box(r.unwrap());
        })
    });
    group.finish();
...
fn bench_sort(c: &mut Criterion) {
    ...
    group.bench_function("sort_multi_key", |b| {
        b.iter(|| {
            let r = conn.query(black_box("MATCH (p:Person) RETURN p.ID, p.name, p.age ORDER BY p.age, p.score"));
            black_box(r.unwrap());
        })
    });
    group.bench_function("sort_limit_top10", |b| {
        b.iter(|| {
            let r = conn.query(black_box("MATCH (p:Person) RETURN p.ID, p.name ORDER BY p.score DESC LIMIT 10"));
            black_box(r.unwrap());
        })
    });
    group.finish();
...
fn bench_join(c: &mut Criterion) {
    ...
    group.bench_function("knows_join", |b| {
        b.iter(|| {
            let r = conn.query(black_box("MATCH (a:Person)-[:Knows]->(b:Person) RETURN a.ID, b.ID LIMIT 1000"));
            black_box(r.unwrap());
        })
    });
    group.bench_function("livesin_join", |b| {
        b.iter(|| {
            let r = conn.query(black_box("MATCH (p:Person)-[:LivesIn]->(c:City) RETURN p.name, c.name LIMIT 1000"));
            black_box(r.unwrap());
        })
    });
...
fn bench_complex(c: &mut Criterion) {
    ...
    group.bench_function("filter_agg", |b| {
        b.iter(|| {
            let r = conn.query(black_box("MATCH (p:Person) WHERE p.active = true RETURN COUNT(p), AVG(p.age), SUM(p.score)"));
            black_box(r.unwrap());
        })
    });
