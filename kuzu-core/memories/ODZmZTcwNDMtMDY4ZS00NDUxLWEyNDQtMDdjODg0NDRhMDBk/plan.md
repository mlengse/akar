...
**Yang masih kurang:** On-disk HashIndex, WAL recovery on startup, Auto-checkpoint wiring, ShadowFile→transaction lifecycle, LocalStorage→commit pipeline, CLI polish (history, auto-complete, multi-line), Benchmarks, tools/rust_api integration, CI/CD, C++ cleanup.
...
- Grammar: `foreach_clause`, `var_length` rules added
- AST: `Clause::Foreach`, bounds on `EdgePattern`
- Parser: `parse_foreach_clause`, var_length in parse_edge_pattern
- Binder: `bind_foreach`, bound propagation
- Logical: `LogicalForeach` operator
- Physical: `PhysicalForeach`
- Processor: Foreach routing
- Tests: 5 parser tests + 4 integration tests
- Catatan: var-length path masih flat scan (belum recursive extend)
...
### B5. Subquery Support (`CALL { ... }`) ✅ (Completed 2026-06-29)

### C1. CLI Enhancement ✅ (Completed 2026-06-29)
- rustyline 12 with multi-line, history, .mode, tab-completion, .import/.export
- rustyline 12 used (rustyline 13+ needs rustc 1.88)
- home@0.5.11 pinned for rustc 1.87 compat
- `pub fn catalog()` added to Database for CLI access
...
### C2. tools/rust_api Integration ✅ (Completed 2026-06-29)
- **Final:** Pure Rust via kuzu-main. C++ FFI (cxx, cmake, C++ headers) removed.
- `Cargo.toml`: required kuzu-main + kuzu-common, no features, no optional deps.
- `build.rs`: just `println!("cargo:rustc-cfg=native")`.
- `src/lib.rs`: always mod native, no dispatch to FFI.
- `src/native.rs`: backward compat API — Error type, Connection::new(&Database) -> Result, Database::new(path, config) -> Result.
- Legacy files removed: include/, ffi-legacy/, lib_ffi.rs, kuzu-src, update_version.py, CMakeLists.txt.
- Verified: cargo build (kuzu + kuzu-rust-example), cargo test --workspace — all pass, 0 failures.
