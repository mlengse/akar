//! Native Azure Blob Storage client for Kuzu.
//!
...
pub fn parse_azure_uri(uri: &str) -> Result<AzureBlobUri, String> {
    ...
    if let Some(rest) = uri.strip_prefix("abfss://").or_else(|| uri.strip_prefix("az://")) {
        ...
        if let Some(at_part) = rest.find('@') {
            ...
            Ok(AzureBlobUri { account, container: container.to_string(), blob })
        } else {
            ...
            match parts.len() {
                ...
                2 => {
                    let account = std::env::var("AZURE_STORAGE_ACCOUNT")
                        .map_err(|_| "Missing AZURE_STORAGE_ACCOUNT env var for az://container/path format".to_string())?;
                    Ok(AzureBlobUri { account, container: parts[0].to_string(), blob: parts[1].to_string() })
                }
                _ => Err(format!("Invalid az URI: {uri}")),
            }
        }
    } else {
        Err(format!("Unsupported Azure URI scheme (expected az:// or abfss://): {uri}"))
    }
}
