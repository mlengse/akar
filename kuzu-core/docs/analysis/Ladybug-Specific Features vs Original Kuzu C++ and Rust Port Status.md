...
**Rust Port Status (`kuzu-core/kuzu-storage/src/ice_format.rs`):**
...
**Rust Port Status (`kuzu-core/kuzu-storage/src/art_index.rs`, `art_key.rs`, `art_node.rs`):**
...
### Rust Port:
- **WAL CREATE_INDEX is PORTED** in `kuzu-storage/src/wal.rs` (WALRecord::CreateIndex { table_id })
...
**Rust Port Status:**
- **ALL FOUR PORTED** in `kuzu-optimizer/src/passes/flat/ladybug.rs` (OrderByPushDown, UnwindDedup, CountRelTable) and `kuzu-optimizer/src/passes/tree/foreign_join.rs` (ForeignJoinPushDown)
...
**Rust Port Status:**
- **PARTIALLY PORTED.** `kuzu-core/kuzu-processor/src/physical/write_ops/packedextend.rs` has a `PhysicalPackedExtend` operator that produces flattened output rows, but the `PackedChildSlices` metadata descriptor and the full packing logic are **NOT ported**.
...
### Rust Port (`kuzu-core/kuzu-transaction/src/lib.rs`):
...
### Rust Port:
- The `adbc.rs` module at `kuzu-core/kuzu-main/src/adbc.rs` provides a basic ADBC wrapper (AdbcDatabase, AdbcConnection, AdbcStatement).
