...
|----------|------|--------|
| `Transaction` | `kuzu-transaction/src/lib.rs` | Txn state: ID, type (READ_ONLY/WRITE), timestamps |
| `TransactionManager` | `kuzu-transaction/src/lib.rs` | Manajer global: begin, commit, rollback, checkpoint |
| `TransactionContext` | `kuzu-transaction/src/lib.rs` | Per-connection: AUTO/MANUAL mode, drop guard |
| `UndoRecord` | `kuzu-storage/src/undo_buffer.rs` | Rollback data: before-image per row |
