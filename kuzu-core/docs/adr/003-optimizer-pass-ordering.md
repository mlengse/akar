...
Query optimizer memiliki 21 pass (14 flat + 7 tree). Urutan eksekusi pass mempengaruhi kualitas final plan.
...
### Flat Pass Order (14 passes)

1. `RemoveUnnecessaryOperators` — bersihkan dulu
2. `FilterPushDown` — push filter sedekat mungkin ke scan
3. `ProjectionPushDown` — eliminasi kolom yang tidak digunakan
4. `ConstantFolding` — evaluasi konstanta
5. `AggregateDetection` — deteksi batas agregasi
6. `JoinOptimization` — DP Bushy Trees join reordering
7. `TopKOptimization` — ORDER BY + LIMIT → TopK scan
8. `VectorSimilarityDetection` — deteksi pola vector similarity
9. `ArtRangeScanDetection` — deteksi ART index range scan
10. `LimitPushDown` — push limit ke bawah
11. `CommonSubexpressionElimination` — eliminasi duplikat
12. `OrderByPushDown` — push ORDER BY di bawah UNION ALL
13. `UnwindDedup` — dedup UNWIND berturut-turut
14. `CountRelTable` — ScanRel+COUNT → CSR metadata
