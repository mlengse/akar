        ...
        #[cfg(feature = "native")]
        {
            ...
            let meta_fn: Arc<dyn Fn(&[Value], &mut kuzu_function::DataChunk) -> Result<(), String> + Send + Sync> =
                Arc::new(|args, chunk| {
                    ...
                    let format_arr = std::sync::Arc::new(arrow::array::StringArray::from(vec![format_version.as_str()])) as arrow::array::ArrayRef;
                    let snap_count_arr = std::sync::Arc::new(arrow::array::Int64Array::from(vec![meta.snapshot_count as i64])) as arrow::array::ArrayRef;
                    let schema_arr = std::sync::Arc::new(arrow::array::StringArray::from(vec![schema_str.as_str()])) as arrow::array::ArrayRef;
                    ...
                    chunk.field_types.push(kuzu_common::types::PhysicalTypeID::String);
                    chunk.field_types.push(kuzu_common::types::PhysicalTypeID::Int64);
                    chunk.field_types.push(kuzu_common::types::PhysicalTypeID::String);
            ...
            let snap_fn: Arc<dyn Fn(&[Value], &mut kuzu_function::DataChunk) -> Result<(), String> + Send + Sync> =
                Arc::new(|args, chunk| {
                    ...
                    chunk.fields.push(
                        std::sync::Arc::new(arrow::array::Int64Array::from(ids)) as arrow::array::ArrayRef
                    );
                    chunk.fields.push(
                        std::sync::Arc::new(arrow::array::Int64Array::from(timestamps)) as arrow::array::ArrayRef
                    );
                    chunk.fields.push(
                        std::sync::Arc::new(arrow::array::StringArray::from(ops)) as arrow::array::ArrayRef
                    );
                    chunk.fields.push(
        ...
        #[cfg(not(feature = "native"))]
        {
            ...
            #[cfg(not(feature = "duckdb-delegation"))]
            {
                context.register_table_function(
                    ...
                    TableFunction::CustomTable {
                        name: "iceberg_scan".into(),
                        execute: Arc::new(|_, _| Err("Iceberg not available (enable feature 'native' or 'duckdb-delegation')".into())),
                    },
                );
                context.register_table_function(
                    "iceberg_metadata",
