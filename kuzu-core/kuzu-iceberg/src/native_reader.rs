//! Native Apache Iceberg table reader for Kuzu.
//!
...
impl IcebergMetadata {
    ...
    pub fn load(table_path: &str) -> Result<Self, String> {
        ...
        let json: serde_json::Value = serde_json::from_str(&content)
            .map_err(|e| format!("Failed to parse Iceberg metadata JSON: {e}"))?;
...
fn parse_schema(json: &serde_json::Value) -> Option<SchemaField> {
    ...
    let schema_val = schemas.iter().find(|s| {
        s.get("schema-id").and_then(|v| v.as_i64()) == Some(current_id)
    })?;

    let fields_arr = schema_val.get("fields")?.as_array()?;
    let fields: Vec<FieldInfo> = fields_arr.iter().map(|f| {
        FieldInfo {
...
fn parse_snapshots(arr: &serde_json::Value) -> Vec<IcebergSnapshot> {
    match arr.as_array() {
        Some(snapshots) => snapshots.iter().map(|s| {
            IcebergSnapshot {
...
mod tests {
    ...
    fn test_format_schema() {
        let schema = SchemaField {
            fields: vec![
                FieldInfo { id: 1, name: "id".into(), type_str: "int".into(), required: true },
                FieldInfo { id: 2, name: "name".into(), type_str: "string".into(), required: false },
            ],
        };
