...
impl PhysicalOperatorExec for PhysicalProjection {
    ...
    fn execute(&self, input: Vec<DataChunk>) -> OperatorResult {
        let output: Vec<DataChunk> = input
            .into_iter()
            .map(|chunk| {
                let fields = self.column_indices.iter().filter_map(|&i| chunk.fields.get(i).cloned()).collect::<Vec<_>>();
                let field_types = self.column_indices.iter().filter_map(|&i| chunk.field_types.get(i).cloned()).collect::<Vec<_>>();
        ...
        if output.is_empty() {
            Ok(vec![DataChunk { fields: vec![], field_types: vec![], 
                size: 0,
                field_names: vec![],
            sel_vector: None,
             }])
