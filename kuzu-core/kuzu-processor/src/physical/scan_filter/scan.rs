...
use arrow::array::{
    Array, BooleanBuilder, Int64Builder, Int32Builder, Float64Builder, Float32Builder, StringBuilder,
    UInt64Array,
};
use arrow::compute;
    ...
    fn execute_with_arrow_arrays(&self, arrays: &[arrow::array::ArrayRef]) -> OperatorResult {
        ...
        if let Some(ref pred) = self.predicate {
            ...
            for &col_idx in &cols_to_scan {
                if col_idx >= arrays.len() { continue; }
        ...
        for &col_idx in &cols_to_scan {
            if col_idx >= arrays.len() { continue; }
            let arr = &arrays[col_idx];
            let ptype = self.table_columns.get(col_idx)
                .map(|c| Self::logical_to_physical(&c.logical_type))
                .unwrap_or(PhysicalTypeID::Int64);
            let taken = compute::take(arr.as_ref(), &indices_arr, None)
                .unwrap_or_else(|_| arr.slice(0, 0));
