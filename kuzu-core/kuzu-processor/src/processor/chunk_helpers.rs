...
pub fn extract_all_rows_from_chunks(chunks: &[kuzu_common::vector::DataChunk]) -> Vec<Vec<Value>> {
...
pub fn extract_all_rows(chunk: &kuzu_common::vector::DataChunk) -> Vec<Vec<Value>> {
...
pub fn rows_to_columns(rows: &[Vec<Value>]) -> (Vec<arrow::array::ArrayRef>, Vec<PhysicalTypeID>) {
    ...
    for col in 0..num_cols {
        ...
        vec.resize(num_rows);
        fields.push(kuzu_common::arrow_vector::ArrowVector::from_legacy(&vec).array);
