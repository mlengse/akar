...
fn main() -> Result<()> {
    ...
    if !args.skip_extract {
        ...
        println!("1. Extracting data and schema from C++ Kuzu (via Python)...");
        let status = Command::new("python")
            .arg(&python_script)
            .arg("--db_path").arg(&args.from)
            .arg("--out_dir").arg(&temp_dir)
            .status()
            .context("Failed to execute python extraction script. Make sure Python and kuzu/ladybug packages are installed.")?;
    ...
    let tables = schema["tables"].as_array().context("Invalid schema: tables is not an array")?;
    let connections = schema["connections"].as_array().context("Invalid schema: connections is not an array")?;
