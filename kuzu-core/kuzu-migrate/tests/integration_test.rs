...
fn test_migration_ingestion() {
    ...
    conn.query(&format!("COPY (MATCH (a:User) RETURN a.id, a.name) TO '{}' (FORMAT PARQUET)", parquet_path_str)).unwrap();

    // Validate the parquet file is readable by performing COPY FROM in the same connection
    conn.query("CREATE NODE TABLE ImportedUser(id INT64, name STRING, PRIMARY KEY(id))").unwrap();
    let import_path_str = parquet_path_str.clone();
    conn.query(&format!("COPY ImportedUser FROM '{}'", import_path_str)).unwrap();
    let result = conn.query("MATCH (u:ImportedUser) RETURN u.id, u.name ORDER BY u.id").unwrap();
    ...
    // Drop DB to release any file locks before running kuzu-migrate
    drop(db);
    drop(conn);
