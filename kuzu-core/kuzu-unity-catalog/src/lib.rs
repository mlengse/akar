        ...
        #[cfg(feature = "native")]
        {
            let scan_fn: Arc<dyn Fn(&[Value], &mut kuzu_function::DataChunk) -> Result<(), String> + Send + Sync> =
                Arc::new(|args, chunk| {
                    ...
                    let name_arr = std::sync::Arc::new(
                        arrow::array::StringArray::from(vec![info.table_name.as_str()])
                    ) as arrow::array::ArrayRef;
                    let type_arr = std::sync::Arc::new(
                        arrow::array::StringArray::from(vec![info.table_type.as_str()])
                    ) as arrow::array::ArrayRef;
                    let schema_arr = std::sync::Arc::new(
                        arrow::array::StringArray::from(vec![info.schema.as_str()])
                    ) as arrow::array::ArrayRef;
                    let location_arr = std::sync::Arc::new(
