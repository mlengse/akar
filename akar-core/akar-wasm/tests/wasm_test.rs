//! WASM integration tests for akar-wasm.
//!
//! Run with: `wasm-pack test --node akar-wasm`

#![cfg(target_arch = "wasm32")]

use akar_wasm::{AkarConnection, AkarDatabase};
...
fn test_create_database() {
    let db = AkarDatabase::new(":memory:").expect("Failed to create in-memory database");
    let conn = AkarConnection::new(&db).expect("Failed to create connection");
...
fn test_query_and_iterate() {
    let db = AkarDatabase::new(":memory:").expect("Failed to create database");
    let conn = AkarConnection::new(&db).expect("Failed to create connection");
