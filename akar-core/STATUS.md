...
| WASM Polish | ⚠️ Basic bindings, no tests | ✅ 6 wasm-bindgen-tests, akar-wasm/README.md, browser target support, wasm-pack compatible | `[P9.5]` |
...
| **P30.1 — Fix 31 Ignored Tests + FTS** | ❌ 32 ignored tests + 1 FTS failure | ✅ **All fixed.** Grammar fixes (`create_rel_table` optional columns, `union_keyword`, `backtick_identifier`), binder relaxations (empty clause count), FTS arrow-path filtering. **1 remaining ignored (akar-migrate — parquet footer, pre-existing).** | `[P30.1]` |
...
| **P30.5a — WASM: fix ignored test** | ❌ 6 test skip di `wasm-pack test --node` (config `run_in_browser`) | ✅ `run_in_browser` → `run_in_node`. `wasm-test` job di CI jalankan `wasm-pack test --node akar-wasm`. | `[P30.5]` |
...
| **P33.2 — gzip VFS** | ❌ No compressed file system | ✅ `GzipFileSystem` in `akar-common` implements `FileSystem` trait, wraps inner FS with `flate2` gzip decoder/encoder. Auto-detects `.gz` extension. | `[P33]` |
| **P33.3 — Progress bar** | ❌ No progress indicator | ✅ `KuzuProgress` wrapper around `indicatif::ProgressBar` in `akar-common/src/progress_bar.rs`. Supports spinner and count-based modes, cancellation flag. | `[P33]` |
| **P33.4 — WAL dump tool** | ❌ No WAL debug utility | ✅ `Display` impl for `WALRecord` (human-readable format). Binary `wal_dump.rs` in `akar-main/src/bin/` — dumps all records from a database WAL file. | `[P33]` |
...
| **P31.4 — akar-migrate parquet** | ✅ Parser `format_option` fix + column name stripping + `parquet-export` feature + test un-ignored | 🟢 **DONE** | `[P31]` |
...
> - ~~**CSR adjacency** (`akar-storage/src/csr.rs`) — `get_neighbors()` return `Ok(vec![])`, belum ada offset/adjacency arrays. RelTable menyimpan `Vec<RelData>` flat sebagai fallback.~~ ✅ **FIXED — P36.1**
> - ~~**Checkpoint** (`akar-storage/src/checkpoint.rs`) — `flush_table()` adalah no-op, tidak benar-benar mem-persist data ke disk.~~ ✅ **FIXED — P36.7**: `flush_table()` now flushes per-file dirty pages, Column metadata persisted via `.meta` sidecar files, 5 new tests verifying full round-trip persistence.
...
|----------|--------|------|
| Frontir (Sparse/Dense/SP) | ✅ | `akar-graph/src/gds/frontier.rs` |
| EdgeCompute, VertexCompute | ✅ | `akar-graph/src/gds/compute.rs` |
| BFSGraph (Dense/Sparse) | ✅ | `akar-graph/src/gds/bfs_graph.rs` |
| OutputWriter (Paths/SP) | ✅ | `akar-graph/src/gds/output_writer.rs` |
| GDSUtils (SSP/ASP/WSP/AWSP) | ✅ | `akar-graph/src/gds/utils.rs` |
| 8 shortest path algorithms | ✅ | `akar-algo/src/lib.rs` |
| PageRank, WCC, SCC, K-Core, Louvain, Spanning Forest | ✅ | `akar-algo/src/lib.rs` |
| Label Propagation, Betweenness Centrality | ✅ | `akar-algo/src/lib.rs` |
| Closeness Centrality, Triangle Counting | ✅ | `akar-algo/src/lib.rs` |
| Random Walk, Node2Vec | ✅ | `akar-algo/src/gds/random_walk.rs`, `node2vec.rs` |
...
| Item | Status |
|------|--------|
| Undo Buffer (rollback safety) | ✅ `akar-storage/src/undo_buffer.rs` |
| WAL Replayer (crash recovery) | ✅ `akar-storage/src/wal_replayer.rs` |
| WALRecord DDL variants (6 new) | ✅ `wal.rs` + `local_wal.rs` |
| Page Manager (FSM integration) | ✅ `akar-storage/src/page_manager.rs` |
...
- **Kuzu Rust** — `akar-core/` → 29 crate, enum definitions, function registry, physical/logical operators
...
|---|-----|----------|----------|--------|
| 1 | ~~**CSR adjacency stub**~~ | ~~🔴 CRITICAL~~ | ~~`akar-storage/src/csr.rs:90`~~ | ~~`get_neighbors()` returns `Ok(vec![])`. RelTable uses flat `Vec<RelData>` fallback.~~ ✅ **FIXED — P36.1**: Full CSR with fwd/rev offsets + adjacency arrays. |
| 2 | ~~**12 DDL operators no-op**~~ | ~~🔴 CRITICAL~~ | ~~`akar-processor/src/physical/write_ops/map_ddl.rs`~~ | ~~CREATE TABLE, DROP TABLE, ALTER TABLE, CREATE/DROP INDEX — all have `execute()` method that returns empty result or does nothing.~~ ✅ **ALL FIXED — P36.3 + P38.1**: All 12 DDL operators now fully wired. P38.1: CreateVectorIndex (akar-vector integration), CreateSequence, DropSequence, CreateDml, ExportDatabase, ImportDatabase via SchemaDdlFn callback. Pk index auto-creation added. |
| 3 | ~~**ORDER BY/LIMIT/SKIP discarded**~~ | ~~🔴 HIGH~~ | ~~`akar-parser/src/ast.rs:227`~~ | ~~`ReturnClause` struct lacks ORDER BY/LIMIT/SKIP fields.~~ ✅ **FIXED — P36.2 + P36.5**: AST fields + planner propagation + PhysicalOrderBy/Limit already existed. |
| 4 | ~~**Binder type resolution hardcoded**~~ | ~~🟡 HIGH~~ | ~~`akar-binder/src/binder/mod.rs:200-250`~~ | ~~Property type lookup uses `match prop_name { "age" => INT64, ... }` heuristic instead of catalog-based schema lookup.~~ ✅ **FIXED — P36.4**: `Catalog::get_property_type()` method + binder now resolves via catalog. Unknown properties return error. |
| 5 | ~~**Checkpoint no-op**~~ | ~~🟡 HIGH~~ | ~~`akar-storage/src/checkpoint.rs`~~ | ~~`flush_table()` is empty function — data never persists to disk beyond in-memory state.~~ ✅ **FIXED — P36.7**: `flush_table()` flushes per-file dirty pages, Column metadata persisted via `.meta` sidecar, 5 tests. |
| 6 | ~~**Pk index wiring (pipeline path)**~~ | ~~🟡 MEDIUM~~ | ~~`akar-processor/src/processor/mapper/map_ddl.rs:47-61`~~ | ~~`CreateNodeTable` in pipeline path doesn't auto-create ART index for PK column.~~ ✅ **FIXED — P38.1**: Pk index auto-creation added to `CreateNodeTable` pipeline operator. |
...
| 2 | **`GREATEST`/`LEAST`** — not implemented | Vela | Fungsi extremum (`GREATEST(a,b,c)` → max value). Tidak ada implementasi Rust (`grep greatest` → 0 hits di `akar-function/`). | Tambah `UtilityOp::Greatest`/`Least`, implement evaluasi + register. **~30 menit** | ✅ **DONE** |
...
| Item | Status |
|------|--------|
| ADBC extension | ✅ `akar-main/src/adbc.rs` — `AdbcDatabase`, `AdbcConnection`, `AdbcStatement`, `AdbcPreparedStatement` (gated `#[cfg(feature = "adbc")]`) |

### 3.5 ✅ Transaction Layer — Multiwriter

| Item | Status |
|------|--------|
| Concurrent write transactions | ✅ `concurrent_writes: AtomicBool` + `writer_condvar: Condvar` di `akar-transaction/src/lib.rs` |
...
### 4.1 Wasm Build & Test
- `akar-wasm` telah **stabil** (Fase P6) dan berhasil di-build menggunakan `wasm-pack` (target `nodejs`).
...
- Artifak NPM sudah siap dan tersedia di `akar-wasm/pkg`.
...
- `akar-json` & `akar-llm`: **Selesai**. Sudah di-wire ke native Rust API via `CustomScalar`.
- `akar-postgres`, `akar-duckdb`: **Selesai**. Sudah fungsional mendelegasikan query atau scanning.
- `akar-azure`, `akar-iceberg`, `akar-delta`, `akar-unity-catalog`: **P34 — Native Readers Done ✅**. DuckDB delegation diganti dengan native Rust readers:
  - `akar-azure`: Native Azure Blob reader via `ureq` HTTP REST + SAS/auth token dari env var
  - `akar-iceberg`: Native metadata reader — parse `metadata.json`, enumerasi `.parquet` data files
  - `akar-delta`: Native Delta log reader — parse `_delta_log/*.json`, resolve `add`/`remove` actions
  - `akar-unity-catalog`: Native REST API client via `ureq` ke Databricks UC `/api/2.1/unity-catalog/tables/`
- `akar-sqlite`, `akar-neo4j`: **Selesai (Native)**.
- `akar-httpfs`: **Selesai (Native)**. HTTP/HTTPS/S3 via VFS Registry + `HttpRandomAccessReader` (HTTP Range requests).
- `akar-fts`: **Selesai (Native)**. Full pipeline: DDL `CREATE FTS INDEX`, MATCH `USING FTS INDEX doc_idx('query')`, BM25 scoring, 3 macro tables (`{name}_docs`, `{name}_terms`, `{name}_appears_in`), Porter stemmer, stop word filtering, tokenizer. Diuji via `akar-main/tests/test_fts.rs`.

### 4.3 P35 — Remaining Minor Gaps (Done ✅)
- **`ConstantOrNullFunction`** ✅ — C++ utility function `CONSTANT_OR_NULL(a,b)` → returns `a` if both non-NULL, else NULL. Implemented as `UtilityOp::ConstantOrNull` in `akar-function`. 5 unit tests.
- **`ConfidentialStatementAnalyzer`** ✅ — LadyDB security feature that prevents confidential `CALL` statements (with S3/Azure secrets) from being stored in shell history. Implemented as `akar-binder::confidential_statement_analyzer` using string-level pattern matching. 5 unit tests. Wired into `akar-cli` to skip `add_history_entry` for confidential CALLs.

### 4.4 Code Quality
- **Clippy: 0 warnings** dengan `-D warnings` — **P32: 29→0 warnings** across 8 crates (akar-common, akar-function, akar-storage, akar-planner, akar-processor, akar-algo, akar-migrate, akar-c). `clippy.toml` configured.
...
|---|------|----------|---------|---------|
| 1 | ~~**Monolith `scalar.rs`** (4.578 lines)~~ | ✅ DONE | ~~`akar-function/src/scalar.rs`~~ → `scalar/{mod,arithmetic,array,blob,boolean,cast,comparison,date,hash,interval,list,map_struct,path,schema,string,union_funcs,utility,utils}.rs` | P-MOD1: ✅ Complete |
| 2 | ~~**Monolith `physical_operator.rs`** (3.794 lines)~~ | ✅ DONE | ~~`akar-processor/src/physical_operator.rs`~~ → `physical/{6 files}` (4-line re-export stub) | P-MOD2A: ✅ Complete |
| 3 | ~~**Monolith `connection.rs`** (3.133 lines)~~ | ✅ DONE | ~~`akar-main/src/connection.rs`~~ → `connection/{mod,query,ddl,dml,copy,transaction,substitute,utils}.rs` | P-MOD3: ✅ Complete (Phase 3) |
| 4 | ~~**Monolith `processor.rs`** (2.755 lines)~~ | ✅ DONE | ~~`akar-processor/src/processor/mod.rs`~~ → `processor/mapper/` (down to ~220 lines) | P-MOD2B: ✅ Complete |
| 5 | ~~**Monolith `passes.rs`** (2.486 lines)~~ | ✅ DONE | ~~`akar-optimizer/src/passes.rs`~~ → `passes/{flat/{11 files},tree/{8 files}}` | P-MOD4: ✅ Complete (Phase 4) |
| 6 | ~~**Monolith `parser.rs`** (2.183 lines)~~ | ✅ DONE | ~~`akar-parser/src/parser.rs`~~ → `parser/{mod,ddl,dml,expression}.rs` | P-MOD5: ✅ Complete (Phase 5) |
| 7 | ~~**Monolith `binder.rs`** (1.667 lines)~~ | ✅ DONE | ~~`akar-binder/src/binder.rs`~~ → `binder/{mod,ddl,dml,helpers}.rs` + `binder_test.rs` | P-MOD6: ✅ Complete (Phase 6) |
| 8 | ~~**TRANSACTION via string matching**~~ | ✅ DONE | ~~`akar-main/src/connection/query.rs`~~ → `Statement::Transaction` pipeline | P10.2 — ✅ Complete |
| 9 | **STANDALONE_CALL dispatch via string matching** | ✅ DONE | `akar-main/src/connection/standalone_call.rs` | P30.4 — Trait `StandaloneCallFn` + registry + 22 handler structs |
| 10 | **Missing physical operators** | 🟡 MEDIUM → 🟢 LOW | `akar-processor/` | P12 — Partitioner, IndexLookup, BatchInsert, dll (TopK ✅ done) |
| 11 | ~~**Missing ATTACH/DETACH DATABASE**~~ | ✅ DONE | Multi-crate | P11 — ✅ Complete (P11.3-5) |
| 12 | ~~**nullif / count_if functions**~~ | ✅ DONE | `akar-function/src/` | P10.5 — ✅ Complete |
| 13 | ~~**EXTENSION mgmt not parsed**~~ | ✅ DONE | `akar-parser/` + `akar-main/` | P10.4 — ✅ Complete |
...
| akar-main (edge_null_handling) | 44 (44 pass, 0 ignore) | ✅ Pass |
| akar-main (edge_boundary) | 20 (20 pass, 0 ignore) | ✅ Pass |
| akar-main (edge_empty_tables) | 17 (17 pass, 0 ignore) | ✅ Pass |
| akar-main (edge_concurrency) | 11 (11 pass, 0 ignore) | ✅ Pass |
| akar-main (edge_ddl_errors) | 21 (21 pass, 0 ignore) | ✅ Pass |
| akar-main (edge_nested_types) | 13 (13 pass, 0 ignore) | ✅ Pass |
| akar-main (edge_unicode) | 11 (11 pass, 0 ignore) | ✅ Pass |
| akar-main (fase_b_verification) | 15 | ✅ Pass |
| akar-main (copy_to) | 4 | ✅ Pass |
| akar-main (delete_set) | 1 | ✅ Pass |
| akar-main (fts) | 1 | ✅ Pass (FIXED) |
...
| akar-neo4j | 12 | ✅ Pass |
| akar-wasm | 3 | ✅ Pass |
| akar-migrate | 1 | ✅ Pass (FIXED — un-ignored) |
...
- Per 2026-07-24: **~1175 test pass, 0 fail, 5 ignored (doc-tests)** ✅. **Sprint 10 COMPLETE — P37.1-P37.4 ALL DONE ✅✅✅✅:** BufferManager mmap/NUMA/readahead, StringDictionary encoding, LadybugDB benchmark suite, Query optimization passes (25 total). **P37.5 ✅ (Production Readiness — LadybugDB C++):** Logger, MetricsRegistry, system_health() table function, ops documentation, 10 production tests. **Sprint 11 COMPLETE — P38-P40 ALL DONE ✅✅✅✅✅:** P38.1 (all 6 remaining DDL operators wired, pk index auto-creation), P38.2 (benchmark verification), P38.3 (documentation polish — MIGRATION.md + rustdoc 50+ items). P39 (Arrow fast path for SUM/AVG/MIN/MAX — ~100× improvement, scalar aggregates at parity with COUNT). P40 (Vectorized GROUP BY with `take()` on ArrayRef — ~37× improvement on GROUP BY + AVG, AggregateDetection correctness bug fix where GROUP BY expressions were silently dropped).
...
- Compile error pada `akar-optimizer` dan clippy warnings terbaru telah diperbaiki.
...
Benchmark file: `akar-main/benches/query_pipeline.rs`. Full report di [`BENCHMARK_COMPARISON.md`](BENCHMARK_COMPARISON.md).

---

## 8. Ladybug C++ Parity Gap Analysis (2026-07-08, updated 2026-07-19)

Audit komparasi penuh antara Rust `akar-core` dan C++ Ladybug (`ladybug/src/`).
...
| Roaring bitmap | P2 | ✅ P17.4 (`akar-storage/src/roaring_bitmap.rs`) — Array/Bitmap containers, union/intersection/difference, 25 tests |
| ICE disk format | P3 | ✅ P20 (`ParquetStreamReader` lazy streaming) |
| Lazy segment scanner | P3 | ✅ P17.3 (`akar-storage/src/lazy_scanner.rs`) — on-demand NodeGroup loading, 6 tests |
...
|-----------|--------|------|
| Dijkstra (SSSP weighted) | ✅ `compute_weighted_shortest_path()` | `akar-algo/src/lib.rs:992` |
| Louvain Community Detection | ✅ `compute_louvain()` | `akar-algo/src/lib.rs:676` |
| K-Core Decomposition | ✅ `compute_k_core()` | `akar-algo/src/lib.rs:509` |
| BFS (unweighted shortest path) | ✅ `compute_shortest_path()` | `akar-algo/src/lib.rs:900` |
| PageRank | ✅ `compute_page_rank()` | `akar-algo/src/lib.rs:339` |
| WCC (Weakly Connected Components) | ✅ `compute_wcc()` | `akar-algo/src/lib.rs:350` |
| SCC (Tarjan + Kosaraju) | ✅ `compute_scc_tarjan()` + `compute_scc_kosaraju()` | `akar-algo/src/lib.rs:362,441` |
| Spanning Forest | ✅ `compute_spanning_forest()` | `akar-algo/src/lib.rs:794` |
| Label Propagation | ✅ `compute_lpa()` | `akar-algo/src/lib.rs:562` |
| Betweenness Centrality | ✅ `compute_betweenness_centrality()` (Brandes) | `akar-algo/src/lib.rs:616` |
| Closeness Centrality | ✅ P17.1 | `akar-algo/src/lib.rs` — 2 tests |
| Triangle Counting | ✅ P17.2 | `akar-algo/src/lib.rs` — 2 tests |
| Random Walk | ✅ CALL `random_walk(steps?, walks?)` | `akar-algo/src/gds/random_walk.rs` — CALL wired |
| Node2Vec / Embedding | ✅ CALL `node2vec(p?, q?, dims?, walks?, window?)` | `akar-algo/src/gds/node2vec.rs` — CALL wired |
...
**Target:** `akar-processor/src/physical_operator.rs` → `akar-processor/src/operators/*.rs`
...
```bash
# Setelah refactor
cargo check -p akar-processor
cargo test -p akar-processor    # Harus tetap 77 passing
...
|---|-------|-----|--------------|
| 1 | IS NULL/IS NOT NULL grammar — negative lookahead `!(ASCII_ALPHANUMERIC \| "_")` gagal karena `WHITESPACE` silent rule sudah consumed space | Merge `is_null_op`/`is_not_null_op` jadi `is_check_op = { "IS" ~ ("NOT" ~ "NULL" \| "NULL") }`, hapus suffix | `akar-parser/src/cypher.pest:355` |
| 2 | Boolean 3VL — NULL propagation override AND/OR short-circuit | Short-circuit `AND` (FALSE if any FALSE) dan `OR` (TRUE if any TRUE) sebelum NULL propagasi di `evaluate_function_call` | `akar-processor/src/expression_evaluator.rs:694-708` |
| 3 | ddl_errors — 8 test assertion string mismatch | Update expected error messages di test body | `akar-main/tests/test_ddl_errors.rs` |
| 4 | Compound type grammar — `INT64[]`, `MAP(...)`, `STRUCT(...)`, `UNION(...)` tidak terdefinisi | Tambah map/struct/union rules + `primitive_type ~ ("[" ~ "]")*` | `akar-parser/src/cypher.pest:247`, `akar-binder/src/binder/mod.rs:109-156` |
...
| # | Issue | Fix | Files Changed |
|---|-------|-----|--------------|
| 5 | NULL PK rejection — `insert_row()` does not check for NULL on primary key column | Add `pk_col.is_null()` check + error return in `table.rs` `insert_row()` and `insert_rows_batch()` | `akar-storage/src/table.rs:430-450` |
| 6 | CASE/COALESCE/IFNULL — grammar `CASE` keyword after `WHEN` not matched due to `atomic(" ") "CASE"` consuming input as atomic sub-rule | Rewrite `case_when`/`coalesce`/`ifnull` rules with `push` + atomic sub-rules to prevent negative-lookahead consumption | `akar-parser/src/cypher.pest:372-410` |
| 7 | CASE/COALESCE/IFNULL — coalesce/ifnull passed through generic NULL propagation (NULL arg → NULL result) also when both branches were NULL | Exempt `coalesce`/`ifnull` from null-propagation in `expression_evaluator.rs`, keeping only `NopInable` path | `akar-processor/src/expression_evaluator.rs:694-708` |
| 8 | Boolean evaluator — 4 new symmetry tests (NULL as second argument) to verify 3VL Kleene logic completeness | Add `test_null_boolean_and_true_other`, `test_null_boolean_and_false_other`, `test_null_boolean_or_true_other`, `test_null_boolean_or_false_other` | `akar-main/tests/test_null_handling.rs:340-380` |
...
cargo bench -p akar-processor       # All benchmarks within target range
