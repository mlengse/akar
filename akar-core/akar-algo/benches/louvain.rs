//! Louvain community detection benchmarks on synthetic community graphs.
...
fn bench_louvain(c: &mut Criterion) {
    ...
    for &size in &[2_000usize, 10_000] {
        ...
        group.bench_with_input(BenchmarkId::new("compute_louvain", size), &csr, |b, csr| {
            b.iter(|| compute_louvain(black_box(csr)))
        });
        group.bench_with_input(BenchmarkId::new("compute_louvain_seeded", size), &csr, |b, csr| {
            b.iter(|| compute_louvain_weighted(black_box(csr), |_u, _v| 1.0, Some(42), 0.001, 20))
        });
    }
    group.finish();
}
