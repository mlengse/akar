        ...
        LogicalOperator::visit_bottom_up(root, &mut |op| {
            if let LogicalOperator::Aggregate(agg) = op {
                ...
                for key in &agg.group_by {
                    if let akar_parser::ast::Expression::PropertyAccess(obj, prop) = key
                        && let akar_parser::ast::Expression::Variable(var) = obj.as_ref()
                ...
                let mut new_keys: Vec<akar_parser::ast::Expression> = Vec::new();
                for key in agg.group_by.drain(..) {
                    let is_dependent = match &key {
                        akar_parser::ast::Expression::PropertyAccess(obj, prop) => {
                            if let akar_parser::ast::Expression::Variable(var) = obj.as_ref() {
