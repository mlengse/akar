...
fn can_inline_projections(outer: &LogicalProjection, inner: &LogicalProjection) -> bool {
    ...
    if let Expression::Variable(name) = &outer.expressions[0].expression {
        inner
            .expressions
            .iter()
            .any(|be| matches!(&be.expression, Expression::Variable(n) if n == name))
    } else {
        false
    }
}
...
fn inline_filter_through_projection(filter: &LogicalFilter, proj: &LogicalProjection) -> Option<LogicalOperator> {
...
fn inline_expression(expr: &Expression, proj_exprs: &[BoundExpression]) -> Expression {
    match expr {
        ...
        Expression::UnaryOp(op, inner) => Expression::UnaryOp(*op, Box::new(inline_expression(inner, proj_exprs))),
        other => other.clone(),
    }
}
