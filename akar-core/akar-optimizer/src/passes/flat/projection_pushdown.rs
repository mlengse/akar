...
fn extract_variables(expr: &akar_parser::ast::Expression, refs: &mut HashSet<String>) {
    match expr {
        akar_parser::ast::Expression::Variable(name) => {
            refs.insert(name.clone());
        }
        akar_parser::ast::Expression::PropertyAccess(obj, _prop) => {
            extract_variables(obj, refs);
        }
        akar_parser::ast::Expression::BinaryOp(_, left, right) => {
            extract_variables(left, refs);
            extract_variables(right, refs);
        }
        akar_parser::ast::Expression::UnaryOp(_, inner) => {
            extract_variables(inner, refs);
        }
        akar_parser::ast::Expression::FunctionCall(_, args) => {
            for arg in args {
        ...
        akar_parser::ast::Expression::Map(entries) => {
            for (_, v) in entries {
                extract_variables(v, refs);
            }
        }
        akar_parser::ast::Expression::Case(case_expr) => {
