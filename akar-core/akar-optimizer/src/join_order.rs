...
fn extract_equality_join(expr: &akar_parser::ast::Expression) -> Option<(String, String)> {
    match expr {
...
fn extract_root_var(expr: &akar_parser::ast::Expression) -> Option<String> {
    match expr {
        akar_parser::ast::Expression::Variable(name) => Some(name.clone()),
        akar_parser::ast::Expression::PropertyAccess(obj, _) => extract_root_var(obj),
        _ => None,
    }
}
...
pub fn reorder_joins_greedy(root: &LogicalOperator) -> Option<Vec<LogicalOperator>> {
    ...
    for (left_alias, right_alias) in &join_conditions {
        let expr = akar_parser::ast::Expression::BinaryOp(
            ...
            Box::new(akar_parser::ast::Expression::Variable(left_alias.clone())),
            Box::new(akar_parser::ast::Expression::Variable(right_alias.clone())),
        );
        result_ops.push(LogicalOperator::Filter(akar_planner::logical_operator::LogicalFilter {
            expression: expr,
            children: Vec::new(),
            cardinality: 0,
        }));
...
fn extract_join_keys_recursive(op: &LogicalOperator, keys: &mut Vec<akar_parser::ast::Expression>) {
    match op {
        LogicalOperator::HashJoin(hj) => {
...
fn build_optimal_tree(
    ...
    original_join_keys: &[akar_parser::ast::Expression],
    ...
    for (left_var, right_var) in conditions {
        if let (Some(&i), Some(&j)) = (alias_to_idx.get(left_var), alias_to_idx.get(right_var)) {
            ...
            if (left_in_left && right_in_right) || (left_in_right && right_in_left) {
                let key_expr = akar_parser::ast::Expression::BinaryOp(
                    akar_parser::ast::BinaryOp::Equal,
                    Box::new(akar_parser::ast::Expression::Variable(left_var.clone())),
                    Box::new(akar_parser::ast::Expression::Variable(right_var.clone())),
                );
                if !join_keys.contains(&key_expr) {
...
mod tests {
    ...
    fn test_reorder_joins_dp_prefers_join_over_cross_product() {
        ...
        let filter_ac = LogicalOperator::Filter(akar_planner::logical_operator::LogicalFilter {
            expression: akar_parser::ast::Expression::BinaryOp(
                akar_parser::ast::BinaryOp::Equal,
                Box::new(akar_parser::ast::Expression::Variable("a".into())),
        ...
        let filter_bc = LogicalOperator::Filter(akar_planner::logical_operator::LogicalFilter {
            expression: akar_parser::ast::Expression::BinaryOp(
                akar_parser::ast::BinaryOp::Equal,
                Box::new(akar_parser::ast::Expression::Variable("b".into())),
                Box::new(akar_parser::ast::Expression::Variable("c".into())),
