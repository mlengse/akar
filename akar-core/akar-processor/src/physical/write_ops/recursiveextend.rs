...
impl PhysicalOperatorExec for PhysicalRecursiveExtend {
    ...
    fn execute(&self, input: Vec<DataChunk>) -> OperatorResult {
        use akar_common::enums::ExtendDirection;
        use akar_common::enums::PathSemantic;
        ...
        let source_offsets: Vec<i64> = if input.is_empty() || input[0].fields.is_empty() {
            ...
            for i in 0..num_rows {
                if !field.is_null(i) {
                    let offset = if let Some(Value::Int64(val)) = input[0].get_value(0, i) {
                        val
                    } else {
                        0
                    };
        ...
        let mut src_v = ValueVector::new(akar_common::types::PhysicalTypeID::Int64, num_results);
        let mut dst_v = ValueVector::new(akar_common::types::PhysicalTypeID::Int64, num_results);
        let mut len_v = ValueVector::new(akar_common::types::PhysicalTypeID::Int64, num_results);
        ...
        let mut path_nodes_v = ValueVector::new(akar_common::types::PhysicalTypeID::List, num_results);
        ...
        if has_cost {
            let mut cost_v = ValueVector::new(akar_common::types::PhysicalTypeID::Double, num_results);
            ...
            Ok(vec![DataChunk {
                fields: vec![
                    akar_common::arrow_vector::ArrowVector::from_legacy(&src_v).array,
                    akar_common::arrow_vector::ArrowVector::from_legacy(&dst_v).array,
                    akar_common::arrow_vector::ArrowVector::from_legacy(&len_v).array,
                    akar_common::arrow_vector::ArrowVector::from_legacy(&path_nodes_v).array,
                    akar_common::arrow_vector::ArrowVector::from_legacy(&path_edges_v).array,
                    akar_common::arrow_vector::ArrowVector::from_legacy(&cost_v).array,
                ],
                field_types: vec![
                    src_v.physical_type(),
                    dst_v.physical_type(),
                    len_v.physical_type(),
                    path_nodes_v.physical_type(),
                    path_edges_v.physical_type(),
                    cost_v.physical_type(),
                ],
                size: num_results,
                field_names: vec![],
                sel_vector: None,
...
impl PhysicalCreateNode {
    pub fn execute(&self, input: Vec<DataChunk>) -> Result<Vec<DataChunk>, ProcessorError> {
        ...
        for mut chunk in input {
            let mut node_ids = ValueVector::new(akar_common::types::PhysicalTypeID::Int64, chunk.size);

            for i in 0..chunk.size {
                let mut values = vec![akar_common::types::Value::Null; table.columns.len()];
            ...
            chunk
                .fields
                .push(akar_common::arrow_vector::ArrowVector::from_legacy(&node_ids).array);
            chunk.field_types.push(akar_common::types::PhysicalTypeID::List);
    ...
    pub fn execute(&self, input: Vec<DataChunk>) -> Result<Vec<DataChunk>, ProcessorError> {
        ...
        for chunk in input {
            ...
            for i in 0..chunk.size {
                ...
                let src_id = if let Some(akar_common::types::Value::Int64(val)) = chunk.get_value(bound_idx, i) {
                    val as u64
                } else {
                    continue;
                };
