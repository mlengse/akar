    ...
    fn batch_group_by_agg(&self, input: &[DataChunk], col_indices: &[Option<usize>]) -> OperatorResult {
        ...
        for (ci, chunk) in input.iter().enumerate() {
            ...
            for row in 0..chunk.size {
                ...
                let entry = group_map
                    .entry(hash)
                    .or_insert_with(|| (build_group_key(chunk, group_cols, row), Vec::new()));
        ...
        let concatenated: Vec<ArrayRef> = if input.len() > 1 {
            col_indices
                ...
                .map(|&ci| {
                    if let Some(col) = ci {
                        let refs: Vec<&dyn Array> = input
                            .iter()
                            .filter_map(|c| {
                                if col < c.fields.len() {
                                    Some(c.fields[col].as_ref())
                                } else {
                                    None
                                }
                            })
                        ...
                        if let Some(col_idx) = col_indices[fi] {
                            ...
                            group_states[fi] = match sub.data_type() {
                                arrow::datatypes::DataType::Int64 => {
                                    ...
                                    match func {
                                        AggregateFunction::Count => AggValueState::Count(group_size),
                                        AggregateFunction::Sum => AggValueState::Sum(
                                            arrow::compute::sum(arr).map(Value::Int64).unwrap_or(Value::Null),
                                        ),
                                        AggregateFunction::Min => AggValueState::Min(
                                            arrow::compute::min(arr).map(Value::Int64).unwrap_or(Value::Null),
                                        ),
                                        AggregateFunction::Max => AggValueState::Max(
                                            arrow::compute::max(arr).map(Value::Int64).unwrap_or(Value::Null),
                                        ),
                                        AggregateFunction::Avg => {
                                            ...
                                            if actual_count == 0 {
                                                ...
                                                AggValueState::Avg {
                                                    sum: Value::Double(s / actual_count as f64),
                                                    count: actual_count,
                                                }
                                            }
                                        }
                                ...
                                arrow::datatypes::DataType::Float64 => {
                                    ...
                                    match func {
                                        AggregateFunction::Count => AggValueState::Count(group_size),
                                        AggregateFunction::Sum => AggValueState::Sum(
                                            arrow::compute::sum(arr).map(Value::Double).unwrap_or(Value::Null),
                                        ),
                                        AggregateFunction::Min => AggValueState::Min(
                                            arrow::compute::min(arr).map(Value::Double).unwrap_or(Value::Null),
                                        ),
                                        AggregateFunction::Max => AggValueState::Max(
                                            arrow::compute::max(arr).map(Value::Double).unwrap_or(Value::Null),
                                        ),
                                        AggregateFunction::Avg => {
                                            let non_null = arr.null_count() as u64;
                                            let actual_count = group_size - non_null;
...
fn arrow_scalar_agg_scan(chunks: &[DataChunk], col_idx: usize, func: &AggregateFunction) -> Value {
    let all_values: Vec<Value> = chunks
        ...
        .flat_map(|c| {
            ...
            (0..c.size)
                .filter_map(|row| c.get_value(col_idx, row))
                .collect::<Vec<_>>()
        })
        .collect();
                ...
                let count = chunks
                    ...
                    .map(|c| {
                        if col_idx >= c.fields.len() {
                            ...
                            (c.fields[col_idx].len() - c.fields[col_idx].null_count()) as u64
                        }
                    })
                    .sum::<u64>();
                if count == 0 {
...
fn agg_val_to_state(func: &AggregateFunction, val: &Value) -> AggValueState {
    match func {
        ...
        AggregateFunction::Avg => match val {
            Value::Double(s) => AggValueState::Avg {
                sum: Value::Double(*s),
                count: 1,
            },
            Value::Int64(s) => AggValueState::Avg {
                sum: Value::Int64(*s),
                count: 1,
            },
            _ => AggValueState::Avg {
