...
> **Revision:** 2026-08-07 (Sprint 18 **IN PROGRESS** — P50.1/P50.2 DONE, P50.3/P50.4 via publish nyata: **11/18 published**, rate-limit 429 di optimizer; gate hijau **1,535 passed / 0 failed**; worktree di-commit d4b3035 & pushed; `akar-graph` di-flip publish=true; detail di tabel P50)
...
> **Catatan scope:** workspace punya **18 crate publishable** (17 dengan `publish = true` + `akar-graph` yang di-flip ke `publish = true` 2026-08-07 karena hard-dep `akar-algo`). 17 asli: akar-common, akar-parser, akar-storage, akar-transaction, akar-catalog, akar-binder, akar-planner, akar-optimizer, akar-processor, akar-function, akar-algo, akar-fts, akar-extension, akar-vector, akar-main, akar-server, akar-wasm. **14 crate sengaja `publish = false`** (kebijakan, bukan blocker teknis): `akar-c` (FFI cdylib, build lokal saja), binary-only `akar-cli`/`akar-migrate`, dan extension/connector `akar-json`, `akar-httpfs`, `akar-azure`, `akar-duckdb`, `akar-delta`, `akar-iceberg`, `akar-sqlite`, `akar-postgres`, `akar-neo4j`, `akar-llm`, `akar-unity-catalog` — semuanya bisa dipublish (dep sudah terpublish) tapi ditahan: sebagian membawa native/optional dep berat (duckdb bundled, rusqlite bundled, tokio-postgres) dan wiring feature (`native`/`duckdb-delegation`) belum final untuk public API; `fuzz` (internal). Publish **bottom-up** mengikuti dependency graph.
...
| P50.3 | **`cargo publish --dry-run`** — validasi tiap crate bisa dikemas & di-compile dari packaged source (bukan dari workspace checkout); perbaiki packaging error sampai bersih; pastikan `akar-main` tidak mengekspos benchmark/test-only deps ke publik. | semua crate, CI script publish | 🔄 IN PROGRESS — **dry-run terverifikasi via publish nyata** (2026-08-07): tiap publish mengemas + meng-compile packaged source vs deps registry yang sudah ter-publish (`akar-vector` download `akar-extension` 0.1.0; `akar-storage` download transaction/vector; `akar-planner` download binder; `akar-optimizer` download planner/storage — verify PASS, upload kena rate-limit 429). `cargo package --list` 32/32 OK, LICENSE ikut ter-package, tak ada file rahasia/path lokal. `akar-main` memiliki optional deps ke crate `publish = false` (json/duckdb/delta/dst) → verify publish `akar-main` harus jalan dengan default features (tanpa `--all-features`); jika gagal, batasi feature yang diaktifkan saat publish |
| P50.4 | **Publish ke crates.io (bottom-up)** — publish berurutan leaf→root (`akar-common` dulu, `akar-main` terakhir) dengan verifikasi tiap crate ter-resolve setelah dipublish; jadikan `akar-main` sebagai entry point utama. | crates.io, git tag `v0.1.0` | 🔄 IN PROGRESS — **11/18 published** (2026-08-07, terverifikasi di crates.io, semua `0.1.0`): `akar-common`, `akar-parser`, `akar-transaction`, `akar-catalog`, `akar-function`, `akar-binder`, `akar-extension`, `akar-vector`, `akar-storage`, `akar-planner`, `akar-fts`. Tiap publish meng-compile packaged source vs deps yang sudah ter-publish di registry (bukti normalize path→registry). **Rate-limit crates.io**: 5 crate baru/periode → hit 429 di `akar-optimizer` (verify OK, upload ditolak); retry setelah window (2026-08-07 16:53:24Z). **Konflik `akar-algo` → `akar-graph`**: `akar-algo` (publish=true) hard-dep `akar-graph` yang tadinya `publish = false` → di-flip ke `publish = true` 2026-08-07 (dep graph = common/storage, sudah publish). **Sisa publish:** optimizer (retry window) → processor → graph → algo → main → server → wasm. **Aman (no version skew):** diff worktree ke crate yang sudah ter-publish murni rustfmt/formatting — tanpa perubahan semantik, jadi 0.1.0 yang beredar tetap valid |
...
**Gate:** `cargo test --workspace` tetap hijau (1,535 passed / 0 failed) setelah setiap publish; `cargo publish --dry-run` bersih untuk semua 18 crate; tag v0.1.0 terverifikasi build dari source yang dipublish.
**Prereq publish berikutnya (2026-08-07):** commit worktree (DONE d4b3035, pushed ke origin/main); publish `akar-optimizer` retry setelah window rate-limit 16:53:24Z; `akar-graph` sudah di-flip `publish = true` (uncommitted — ikut commit berikutnya).
...
| **Sprint 18** | **P50 Release Readiness & Publishing** | **~5** | 🔜 IN PROGRESS — audit metadata/license, version baseline 0.1.0, publish nyata (bottom-up, **11/18 published**), rate-limit 429 di optimizer (retry 16:53:24Z), `akar-graph` di-flip publish=true (dep akar-algo), tag v0.1.0 + release notes |
