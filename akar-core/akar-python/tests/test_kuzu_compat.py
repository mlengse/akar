"""P53.9 — Akar store compat harness.

Runs the REAL Kairos stores — ``AkarStore`` and ``AkarDreamBackend``
(``kairos/akar_store.py``, ``kairos/dream_akar_store.py``) — against
``import akar``, via module aliases registered in ``sys.modules`` before any
kairos module is imported. Every test uses a fresh temp database so failures
are isolated and map 1:1 to a store method.
...
from kairos.akar_store import AkarStore  # noqa: E402, PLC0415
from kairos.dream_akar_store import AkarDreamBackend  # noqa: E402, PLC0415
...
# ---------------------------------------------------------------------------
@pytest.fixture
def store(tmp_path: Path) -> AkarStore:
    s = AkarStore(db_path=str(tmp_path / "mem.db"), dimension=DIM)
    yield s
    s.close()


@pytest.fixture
def dream(store: AkarStore) -> AkarDreamBackend:
    d = AkarDreamBackend(store=store)
    yield d
    d.close()


@pytest.fixture
def seeded_store(store: AkarStore) -> AkarStore:
...
class TestSchemaBootstrap:
    def test_core_tables_exist(self, store: AkarStore) -> None:
    ...
    def test_metadata_tables_exist(self, store: AkarStore) -> None:
        rows = store._query_all("CALL show_tables()")
        names = {str(r.get("col_0") or r.get("name") or "") for r in rows}
    ...
    def test_bootstrap_is_idempotent(self, store: AkarStore) -> None:
        store._ensure_schema()
        rows = store._query_all("CALL show_tables()")
        names = {str(r.get("col_0") or r.get("name") or "") for r in rows}
        assert "Memory" in names and "Counter" in names

    def test_metadata_rel_tables_exist(self, store: AkarStore) -> None:
        rows = store._query_all("CALL show_tables()")
        names = {str(r.get("col_0") or r.get("name") or "") for r in rows}
...
class TestStore:
    def test_store_and_get(self, store: AkarStore) -> None:
    ...
    def test_store_with_explicit_id(self, store: AkarStore) -> None:
        mid = store.store("a", "content a", _embed(2), id_=42, content_hash="h-42")
        assert mid == 42
        m = store.get(42)
        assert m is not None and m["id"] == 42

    def test_store_dedup_by_content_hash(self, store: AkarStore) -> None:
    ...
    def test_store_many(self, store: AkarStore) -> None:
    ...
    def test_get_missing_returns_none(self, store: AkarStore) -> None:
        assert store.get(9999) is None

    def test_get_many(self, seeded_store: AkarStore) -> None:
    ...
    def test_get_all(self, seeded_store: AkarStore) -> None:
        rows = seeded_store.get_all()
        assert len(rows) == 3
        assert [r["id"] for r in rows] == [1, 2, 3]
        assert all(len(r["embedding"]) == DIM for r in rows)

    def test_list_ids(self, seeded_store: AkarStore) -> None:
        assert seeded_store.list_ids() == [1, 2, 3]

    def test_find_by_content_hash(self, store: AkarStore) -> None:
    ...
    def test_find_by_label(self, seeded_store: AkarStore) -> None:
        rows = seeded_store.find_by_label("mem2")
        assert len(rows) == 1 and rows[0]["id"] == 2

    def test_get_max_id(self, seeded_store: AkarStore) -> None:
        assert seeded_store.get_max_id() == 3

    def test_count(self, seeded_store: AkarStore) -> None:
        assert seeded_store.count() == 3

    def test_revision_count(self, store: AkarStore) -> None:
        assert store.revision_count() == 0

    def test_add_revision(self, store: AkarStore) -> None:
...
class TestConnections:
    def test_add_connection(self, seeded_store: AkarStore) -> None:
    ...
    def test_add_connections_batch(self, seeded_store: AkarStore) -> None:
        seeded_store.add_connections_batch([(1, 3), (2, 3)])
        conns = seeded_store.get_connections(1)
        assert any(c["target"] == 3 for c in conns)
        assert seeded_store.connection_count() == 3

    def test_get_connections(self, seeded_store: AkarStore) -> None:
        conns = seeded_store.get_connections(1)
        assert len(conns) == 1
    ...
    def test_get_connections_at_time(self, seeded_store: AkarStore) -> None:
        conns = seeded_store.get_connections(1, at_time=2000.0)
        assert len(conns) == 1 and conns[0]["target"] == 2

    def test_get_all_connections(self, seeded_store: AkarStore) -> None:
        conns = seeded_store.get_all_connections()
        assert len(conns) == 1 and conns[0]["target"] == 2

    def test_top_weighted_edges(self, seeded_store: AkarStore) -> None:
        edges = seeded_store.top_weighted_edges(10)
        assert len(edges) == 1 and edges[0]["weight"] == 0.9

    def test_connection_count(self, seeded_store: AkarStore) -> None:
        assert seeded_store.connection_count() == 1


class TestUpdateAndMetadata:
    def test_update_memory_content(self, seeded_store: AkarStore) -> None:
    ...
    def test_touch(self, seeded_store: AkarStore) -> None:
        seeded_store.touch(1)
        m = seeded_store.get(1)
        assert m is not None and m["access_count"] == 1

    def test_set_meta_get_meta(self, store: AkarStore) -> None:
        store.set_meta("k", "v")
        assert store.get_meta("k") == "v"


class TestSearch:
    def test_search_semantic_returns_candidates(self, seeded_store: AkarStore) -> None:
        results = seeded_store.search_semantic(_embed(1), limit=10)
        assert len(results) >= 1
        assert results[0]["channel"] == "semantic"
        assert results[0]["id"] in (1, 2, 3)

    def test_search_semantic_similarity_score(self, seeded_store: AkarStore) -> None:
        results = seeded_store.search_semantic(_embed(1), limit=10)
        assert results and results[0]["id"] == 1
        assert results[0]["similarity"] > 0.0

    def test_search_bm25(self, seeded_store: AkarStore) -> None:
        results = seeded_store.search_bm25("hello", limit=10)
        assert len(results) == 3
        assert all(r["channel"] == "bm25" for r in results)

    def test_search_entity(self, seeded_store: AkarStore) -> None:
        results = seeded_store.search_entity("mem2", limit=10)
        assert len(results) == 1 and results[0]["id"] == 2

    def test_search_temporal(self, seeded_store: AkarStore) -> None:
...
class TestMaintenance:
    def test_export_database(self, store: AkarStore, tmp_path: Path) -> None:
    ...
    def test_repair_schema(self, seeded_store: AkarStore) -> None:
        stats = seeded_store.repair_schema()
        assert stats["ok"] is True, stats
        assert seeded_store.count() == 3

    def test_store_many_empty_returns_empty(self, store: AkarStore) -> None:
        assert store.store_many([]) == []
        assert store.count() == 0

    def test_empty_store_many_does_not_panic_get_all(self, store: AkarStore) -> None:
        store.store_many([])
        rows = store.get_all()
        assert rows == []

    def test_close_and_reopen(self, store: AkarStore, tmp_path: Path) -> None:
        path = str(tmp_path / "mem.db")
        s = AkarStore(db_path=path, dimension=DIM)
        s.store("a", "persist me", _embed(1), id_=1)
        s.close()
        s2 = AkarStore(db_path=path, dimension=DIM)
        try:
            m = s2.get(1)
...
class TestDream:
    def test_dream_schema_tables_exist(self, dream: AkarDreamBackend) -> None:
    ...
    def test_start_finish_session(self, dream: AkarDreamBackend) -> None:
        sid = dream.start_session("nrem")
        assert sid >= 1
        dream.finish_session(sid, {"processed": 5, "strengthened": 2})

    def test_get_dream_stats(self, dream: AkarDreamBackend) -> None:
        stats = dream.get_dream_stats()
        assert stats["sessions"] == 0

    def test_add_insight(self, dream: AkarDreamBackend) -> None:
        sid = 1
        dream.add_insight(sid, "cluster", 2, "a cluster insight", 0.9)

    def test_add_insights_batch(self, dream: AkarDreamBackend) -> None:
        n = dream.add_insights_batch([(1, "cluster", 2, "insight A", 0.9), (1, "pattern", 3, "insight B", 0.8)])
        assert n == 2

    def test_log_connection_change(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        dream.log_connection_change(1, 2, 0.5, 0.8, "nrem_strengthen", 7)

    def test_strengthen_connection(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        dream.strengthen_connection(1, 2, delta=0.05)
        conns = seeded_store.get_connections(1)
        assert conns[0]["weight"] == 0.9 + 0.05

    def test_batch_strengthen_connections(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
    ...
    def test_set_connection_weight(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        assert dream.set_connection_weight(1, 2, 0.25) is True
        assert seeded_store.get_connections(1)[0]["weight"] == 0.25

    def test_add_bridge(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        assert dream.add_bridge(1, 3, weight=0.3) is True
        targets = {c["target"] for c in seeded_store.get_connections(1)}
        assert 3 in targets

    def test_add_supersedes_batch(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        n = dream.add_supersedes_batch([(1, 3, 0.5)])
        assert n == 1

    def test_prune_weak(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        dream.set_connection_weight(1, 2, 0.02)
        pruned = dream.prune_weak(threshold=0.05)
        assert pruned == 1
        assert seeded_store.connection_count() == 0

    def test_prune_connection_history(self, dream: AkarDreamBackend) -> None:
        assert dream.prune_connection_history(keep_days=7) == 0

    def test_recent_cluster_anchors(self, seeded_store: AkarStore, dream: AkarDreamBackend) -> None:
        dream.add_insight(1, "cluster", 2, "c", 0.9)
        anchors = dream.recent_cluster_anchors(window_seconds=3600)
        assert 2 in anchors

    def test_close(self, dream: AkarDreamBackend) -> None:
        dream.close()
