//! Type converter between DuckDB values and Akar types/values.
//!
//! Maps DuckDB types (INTEGER, BIGINT, VARCHAR, DOUBLE, BOOLEAN, etc.)
//! to Akar `LogicalTypeID` and `Value` types.

use akar_common::types::LogicalTypeID;
#[cfg(feature = "bundled")]
use akar_common::types::Value;

/// Convert a DuckDB logical type string to a Akar `LogicalTypeID`.
pub fn duckdb_type_to_akar(duckdb_type: &str) -> LogicalTypeID {
...
/// Convert a duckdb::types::Value to a Akar Value.
#[cfg(feature = "bundled")]
pub fn duckdb_value_to_akar(duck_val: &duckdb::types::Value) -> Value {
...
mod tests {
    ...
    #[test]
    fn test_duckdb_type_to_akar() {
        assert_eq!(duckdb_type_to_akar("INTEGER"), LogicalTypeID::Int32);
        assert_eq!(duckdb_type_to_akar("BIGINT"), LogicalTypeID::Int64);
        assert_eq!(duckdb_type_to_akar("VARCHAR"), LogicalTypeID::String);
        assert_eq!(duckdb_type_to_akar("DOUBLE"), LogicalTypeID::Double);
        assert_eq!(duckdb_type_to_akar("BOOLEAN"), LogicalTypeID::Bool);
        assert_eq!(duckdb_type_to_akar("FLOAT"), LogicalTypeID::Float);
        assert_eq!(duckdb_type_to_akar("DATE"), LogicalTypeID::Date);
        assert_eq!(duckdb_type_to_akar("TIMESTAMP"), LogicalTypeID::Timestamp);
        assert_eq!(duckdb_type_to_akar("BLOB"), LogicalTypeID::Blob);
        assert_eq!(duckdb_type_to_akar("UUID"), LogicalTypeID::Uuid);
    }

    #[test]
    fn test_duckdb_type_to_akar_unknown() {
        assert_eq!(duckdb_type_to_akar("UNKNOWN_TYPE"), LogicalTypeID::String);
    }
}
