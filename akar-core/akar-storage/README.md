...
Page-based columnar storage engine with buffer management, WAL, compression, indexing,
and data readers.

**Buffer Manager:** Clock eviction policy, composite-key page mapping
`(file_name, page_num)`, configurable pool size with dirty-page flushing.
...
**NodeTable/RelTable:** NodeGroup-based storage with insert_row, delete_row, update_cell,
scan_column. RelTable uses CSR adjacency lists (label-keyed) + per-column property storage.
...
**CSV Reader:** Full CSV parser with header detection, custom delimiter/quote/escape, type
coercion for all 28+ Value variants, error reporting with line numbers.

**Parquet Reader:** Parquet file reader using Apache Arrow/Parquet crates (v53). Row group
reading, Arrow->Akar type mapping (Int64->INT64, Utf8->STRING, etc.).
