    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            NpyReaderError::ShapeMismatch { expected_rows, actual } => write!(
                f,
                "Shape mismatch: expected {} rows, file has {} elements",
...
fn parse_header(data: &[u8]) -> Result<NpyHeader, NpyReaderError> {
    ...
    let descr = extract_py_str(header_str, "descr").unwrap_or("<f8".to_string());
...
fn extract_py_tuple(data: &str, key: &str) -> Option<Vec<usize>> {
    ...
    let nums: Vec<usize> = inner.split(',').filter_map(|s| s.trim().parse().ok()).collect();
