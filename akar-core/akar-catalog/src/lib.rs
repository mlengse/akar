...
impl Catalog {
    ...
    pub fn connection_info(&self, table_name: &str) -> Option<Vec<akar_common::types::Value>> {
        let entry = self.get_entry_by_name(table_name)?;
        match entry {
            CatalogEntry::NodeTable(nt) => Some(vec![
                akar_common::types::Value::String(nt.name.clone()),
                akar_common::types::Value::String("NODE".to_string()),
            ...
            CatalogEntry::RelTable(rt) => {
                ...
                Some(vec![
                    akar_common::types::Value::String(rt.name.clone()),
                    akar_common::types::Value::String(src),
                    akar_common::types::Value::String(dst),
                ])
            }
            _ => None,
        }
