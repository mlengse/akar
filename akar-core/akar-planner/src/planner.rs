...
/// AND-combine a newly bound WHERE predicate with any previously accumulated
/// filter (e.g. an implicit one generated by the binder from inline
/// node-properties) — otherwise the earlier predicate would be silently
/// dropped (P48.17 BUG-A). With no prior filter, the new predicate is kept
/// verbatim.
fn combine_filter_expr(prev: Option<BoundExpression>, next: BoundExpression) -> BoundExpression {
    let Some(prev) = prev else { return next };
    BoundExpression {
        expression: Expression::BinaryOp(
            akar_parser::ast::BinaryOp::And,
            Box::new(prev.expression),
            Box::new(next.expression),
        ),
        resolved_type: akar_common::types::LogicalTypeID::Bool,
        is_constant: prev.is_constant && next.is_constant,
        alias: None,
    }
}
            ...
            match clause {
                ...
                BoundClause::BoundWhere(w) => {
                    if !delete_exprs.is_empty() {
                        ...
                        // Combine with any prior WHERE clause — see `combine_filter_expr`
                        // (P48.17 BUG-A).
                        filter_expr = Some(combine_filter_expr(filter_expr.take(), w.expression));
                    }
                }
