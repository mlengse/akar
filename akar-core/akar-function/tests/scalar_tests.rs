...
use akar_function::aggregate::{AggValueState, evaluate_aggregate};
use akar_function::registry::*;
...
fn test_comparison_nan_equality() {
    ...
    let neq = ScalarFunction::Comparison {
...
fn test_cast_int64() {
    ...
    assert_eq!(evaluate_scalar(&func, &[Value::Int64(42)]).unwrap(), Value::Int64(42));
    assert_eq!(
        evaluate_scalar(&func, &[Value::Double(std::f64::consts::PI)]).unwrap(),
        Value::Int64(3)
    );
}
...
fn test_schema_offset_internal_id() {
    ...
    let id = akar_common::types::InternalID {
        table_id: 1,
        offset: 42,
    };
    assert_eq!(
...
fn test_schema_start_end_node() {
    ...
    let src_id = akar_common::types::InternalID {
        table_id: 1,
        offset: 10,
    };
    let dst_id = akar_common::types::InternalID {
        table_id: 1,
        offset: 20,
    };
...
fn test_schema_label_internal_id() {
    let func = ScalarFunction::Schema { op: SchemaOp::Label };
    let id = akar_common::types::InternalID { table_id: 3, offset: 0 };
