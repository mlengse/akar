...
- `akar-migrate` CLI tool — export C++ DB via Python `kuzu` → Parquet → import ke Rust (`MIGRATION.md:9-19`)
- API publik identik (Database, Connection, QueryResult, Cypher query language)
- Python: `pip install akar-rust`, Node.js: `npm install @vela-engineering/kuzu`
...
| Data migration | ✅ **Satu-arah** via `akar-migrate` (C++ → Parquet → Rust) |
