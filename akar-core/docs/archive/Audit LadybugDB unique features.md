# Comprehensive Audit: LadybugDB C++ vs Kuzu C++ (Vela) vs Rust Port (akar-core) 18/07/2026

## 1. Repository Architecture Overview

| Layer | Ladybug C++ (`ladybug/`) | Kuzu Core Rust (`akar-core/`) | Kuzu C++ Vela (presumed) |
|-------|--------------------------|-------------------------------|--------------------------|
...
| **Shell** | linenoise-based (`lbug_shell`) | rustyline-based (`akar-cli`) | N/A |
...
### 9.8. Extended C API
Ladybug's C API (`c_api/`) exports a comprehensive C interface for language bindings. The Rust port has `akar-c` for C bindings but the API surface may differ.
...
`benchmark/` directory with dedicated benchmark infrastructure. Rust has benchmarks in `akar-main/benches/` but with different structure.
