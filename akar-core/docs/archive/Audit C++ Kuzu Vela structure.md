...
All 14 have corresponding Rust crates in `akar-core/`:
```
akar-algo, akar-azure, akar-duckdb, akar-fts, akar-httpfs,
akar-iceberg, akar-json, akar-llm, akar-neo4j, akar-postgres,
akar-sqlite, akar-unity-catalog, akar-vector, akar-delta
```
...
├── rust_api/           # Rust API (via FFI - OLD, now replaced by akar-core)
├── shell/              # Interactive shell (CLI)
├── stress/             # Stress testing
└── wasm/               # WASM build
```

**Note:** `tools/rust_api/` exists in C++ but is the OLD FFI-based Rust binding. The NEW Rust port in `akar-core/` is a pure-Rust rewrite.
...
### Rust Physical Operators (from akar-processor/src/ files)
...
After thorough analysis comparing the C++ source tree against the Rust `akar-core/` source tree:
...
| Virtual File System (VFS) | ✅ Ported | `akar-storage` has VFS |
| Compressed file system | ❓ Unknown | `compressed_file_system.h` - may not be ported |
| NPY reader | ✅ Ported | `akar-storage/src/npy_reader.rs` |
| Parquet writer | ✅ Ported | `akar-storage/src/parquet_writer.rs` |
| Thread pool/task scheduler | ✅ Ported | `akar-common/src/task_system.rs` |
...
The Rust `akar-processor/src/processor/mapper/` has equivalents.
