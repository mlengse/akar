...
> **Methodology:** Compare C++ Kuzu (v0.12.0), LadybugDB C++ (v0.18.0 fork), against Rust akar-core
...
|---|---------|----------|----------|
| 1 | **CSR adjacency stub** — `get_neighbors()` returns `Ok(vec![])` | 🔴 CRITICAL | `akar-storage/src/csr.rs:90` |
| 2 | **12 DDL operators no-op** — CREATE/DROP/ALTER TABLE return empty chunks | 🔴 CRITICAL | `akar-processor/src/physical/write_ops/map_ddl.rs` |
| 3 | **ORDER BY/LIMIT/SKIP discarded** — parsed but not stored in AST | 🔴 CRITICAL | `akar-parser/src/ast.rs:227` |
| 4 | **Binder type resolution hardcoded** — `name`→String, `age`→Int64 heuristic | 🟡 HIGH | `akar-binder/src/binder/mod.rs:200-250` |
| 5 | **Checkpoint no-op** — `flush_table()` is empty function | 🟡 HIGH | `akar-storage/src/checkpoint.rs` |
...
```
C++ Kuzu (v0.12.0)                     Rust akar-core (v0.1.0)
├── src/                                ├── akar-core/
│   ├── antlr4/ (ANTLR grammar)         │   ├── akar-parser/ (pest.rs PEG)
│   ├── main/                           │   ├── akar-main/
│   ├── common/                         │   ├── akar-common/
│   ├── parser/                         │   ├── akar-parser/
│   ├── binder/                         │   ├── akar-binder/
│   ├── planner/                        │   ├── akar-planner/
│   ├── optimizer/                      │   ├── akar-optimizer/
│   ├── processor/                      │   ├── akar-processor/
│   ├── storage/                        │   ├── akar-storage/
│   ├── catalog/                        │   ├── akar-catalog/
│   ├── transaction/                    │   ├── akar-transaction/
│   ├── function/                       │   ├── akar-function/
│   ├── graph/                          │   ├── akar-graph/
│   ├── expression_evaluator/           │   │   (ada di processor)
│   └── c_api/                          │   ├── akar-c/
├── extension/ (14 modules)             │   ├── akar-{json,fts,vector,...}*
└── tools/                              └── akar-cli/, akar-wasm/, akar-migrate/
```
...
```rust
// akar-storage/src/checkpoint.rs
...
|-----------|-----|------|--------|
| json | Full | `akar-json` | Placeholder |
| fts | Full | `akar-fts` | ✅ Implemented (BM25) |
| vector (HNSW) | Full | `akar-vector` | ✅ Implemented |
| httpfs | Full | `akar-httpfs` | Placeholder |
| duckdb | Full | `akar-duckdb` | Placeholder |
| algo (GDS) | Full | `akar-algo` | ✅ Implemented |
| neo4j | Full | `akar-neo4j` | Placeholder |
| llm | Full | `akar-llm` | Placeholder |
| sqlite | Full | `akar-sqlite` | Placeholder |
| delta | Full | `akar-delta` | Placeholder |
| iceberg | Full | `akar-iceberg` | Placeholder |
| azure | Full | `akar-azure` | Placeholder |
| postgres | Full | `akar-postgres` | Placeholder |
| unity-catalog | Full | `akar-unity-catalog` | Placeholder |
| adbc | Full (Ladybug) | `akar-main/adbc.rs` | Basic wrapper |
