...
fn test_verification_merge_create_new_node() {
    ...
    assert_eq!(names[0], akar_common::types::Value::String("Bob".into()));
}
...
fn test_verification_create_dml_basic() {
    ...
    let names = query_column(&conn, "MATCH (n:Person) RETURN n.name");
    assert_eq!(names.len(), 1, "Should have 1 node");
    assert_eq!(names[0], akar_common::types::Value::String("Alice".into()));
}
...
fn test_verification_foreach_create_nodes() {
    ...
    assert_eq!(vals[0], akar_common::types::Value::Int64(1));
    assert_eq!(vals[1], akar_common::types::Value::Int64(2));
    assert_eq!(vals[2], akar_common::types::Value::Int64(3));
}
...
fn test_verification_foreach_empty_list() {
    ...
    assert_eq!(vals[0], akar_common::types::Value::Int64(99));
}
...
fn test_verification_merge_and_create_and_match() {
    ...
    assert_eq!(names[0], akar_common::types::Value::String("Alice".into()));
    assert_eq!(names[1], akar_common::types::Value::String("Bob".into()));
}
