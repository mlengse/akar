...
fn test_bug() {
    ...
    conn.query("CREATE NODE TABLE A(id INT64, prop INT64, PRIMARY KEY(id))")
        .unwrap();
    conn.query("CREATE NODE TABLE B(id INT64, prop INT64, PRIMARY KEY(id))")
        .unwrap();
    conn.query("CREATE REL TABLE E(FROM A TO B, dummy INT64)").unwrap();

    let nodes = 12;
    for i in 0..nodes {
        conn.query(&format!("CREATE (a:A {{id: {}, prop: {}}})", i, i % 20))
            .unwrap();
        conn.query(&format!("CREATE (b:B {{id: {}, prop: {}}})", i, i % 20))
            .unwrap();
    }
    ...
    for i in 0..edges {
        ...
        conn.query(&format!(
            "MATCH (a:A {{id: {}}}), (b:B {{id: {}}}) CREATE (a)-[:E]->(b)",
            from, to
        ))
        .unwrap();
    }
    let filter_val = 6;
    let q1 = format!(
        "MATCH (a:A)-[:E]->(b:B) WHERE a.prop > {} AND b.prop < {} RETURN a.id, a.prop, b.id, b.prop",
        filter_val,
        filter_val + 5
    );
    ...
    println!(
        "QUERY RESULT ROWS: {}",
        res.chunks.iter().map(|c| c.size).sum::<usize>()
    );
