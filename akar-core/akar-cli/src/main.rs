...
impl CliState {
    ...
    fn execute_dot_command(&mut self, cmd: &str, output: &mut dyn Write) -> Result<(), String> {
        ...
        match parts[0].to_lowercase().as_str() {
            ...
            "help" => {
                writeln!(output, "Akar CLI commands:").ok();
                writeln!(
                    output,
                    "  .mode <mode>    Output: table|csv|json|line|column|box|html|latex"
                )
                .ok();
            ...
            "mode" => {
                if parts.len() < 2 {
                    ...
                    writeln!(
                        output,
                        "Unknown: {}. Options: table, csv, json, line, column, box, html, latex",
                        parts[1]
                    )
                    .ok();
                }
...
fn run_repl_rustyline(output: &mut dyn Write) {
    ...
    writeln!(output, "Akar CLI v{}", env!("CARGO_PKG_VERSION")).ok();
    writeln!(output, "Enter queries (end with ;). Type .help").ok();
    writeln!(output).ok();

    loop {
        let readline = rl.readline("akar> ");
        match readline {
            Ok(line) => {
                ...
                let is_conf = akar_binder::confidential_statement_analyzer::is_confidential_call(clean);
                if !is_conf {
...
fn format_output(result: &akar_main::QueryResult, mode: OutputMode, output: &mut dyn Write) {
    let mut rows: Vec<Vec<String>> = Vec::new();
    for chunk in &result.chunks {
        for r in 0..chunk.size {
            rows.push(
                (0..chunk.fields.len())
                    .map(|col| fmt_val(&chunk.get_value(col, r).unwrap_or(akar_common::types::Value::Null)))
                    .collect(),
            );
        }
    }
...
fn export_to_csv(result: &akar_main::QueryResult, file_path: &str) -> Result<(), String> {
    use std::fs::File;
    use std::io::Write;
    let mut f = File::create(file_path).map_err(|e| format!("Cannot create: {e}"))?;
    for chunk in &result.chunks {
        for r in 0..chunk.size {
            let row: Vec<String> = (0..chunk.fields.len())
                .map(|col| fmt_val(&chunk.get_value(col, r).unwrap_or(akar_common::types::Value::Null)))
                .collect();
            let csv: Vec<String> = row
                .iter()
