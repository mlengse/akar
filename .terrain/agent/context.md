...
Akar is a **pure-Rust, embedded graph database** built to be the durable memory of an AI agent: it stores knowledge-graph facts (nodes/relationships) speakable in an OpenCypher-compatible dialect, alongside dense `float32` vectors so recall becomes similarity search. It reimplements the KuzuDB architecture from scratch in safe Rust with **zero FFI in the core** (ADR-002), embedding in-process: `Database::new(path, config)` → `Connection::query(cypher)` → `QueryResult`, no server round-trip. Consumers are agent memory systems (e.g. PyPI `sulur`), plus a TCP daemon, CLI, C FFI, WASM and Python bindings. Constraints: GPL-3.0-or-later, Rust edition 2024, WASM-viable, strictly layered 35-crate Cargo workspace under `akar-core/` (~106K LOC, **2,086 tests, 0 failed**, gate 2026-09-17). Development is **SPEC-driven** — `SPEC.md` is the living spec; ADRs in `akar-core/docs/adr/`.

## Architecture

| Dimension | Design |
|---|---|
| Query pipeline | Single chain in `Connection::query`: **parse** (pest PEG → AST, 33 `Statement` variants) → **bind** (catalog-aware `Binder` → 33 `BoundStatement`) → **plan** (`QueryPlanner` → 59 `LogicalOperator` variants) → **optimize** (25 rule passes: 18 flat + 7 tree, pinned by test) → **execute** (`QueryProcessor`, 55 physical operators) → `QueryResult` |
| Execution model | Vectorized Volcano: every `PhysicalOperatorExec` maps `Vec<DataChunk>` → `Vec<DataChunk>`; `DataChunk` = Arrow `ArrayRef` columns + physical types + optional `SelectionVector` (row indirection). Hot path uses `ExpressionEvaluator::evaluate_to_arrow` (Arrow compute kernels, no `Value` boxing); parallel agg/hash-join via rayon `TaskSystem`. **F6 memory blow-up mitigation:** Extend column pruning (`ExtendPrune` — conservative downstream-tail analysis materialises only referenced columns; identity `id`/`_id` always kept); `PhysicalCrossProduct` accepts a pushed-down row budget via `execute_binary_budgeted`; unpruned extends/cross-products enforce safety caps (`AKAR_MAX_EXTEND_ROWS` 5M / `AKAR_MAX_CROSS_ROWS` 100K, env-overridable) to prevent OOM |
| Optimizer | Two-phase, ordered, rule-based: flat passes (filter/predicate/projection pushdown, top-k, DP-bushy join reorder, `VectorSimilarityDetection` rewriting cosine→HNSW scan, ART range scan) then 7 bottom-up tree passes (factorization, foreign-join, acc-hash-join, subquery unnesting, agg-key-dependency, cardinality, **FTS predicate pushdown**). `FtsCardinalityEstimator` blends Tantivy `doc_freq` into cardinality (zero-extra-probe) |
| Storage | Column-major: `NodeTable`/`RelTable` → `NodeGroup` (4096 rows) → `ColumnChunk`; `BufferManager` page cache + `PageManager`/`FreeSpaceManager`; append-only typed **WAL** (magic `b"AKAR"`, CRC32, self-sufficient replay) + `WalReplayer` + `ShadowFile`/`Checkpoint`; CSR adjacency for rels; ART PK index, HNSW vector index, hash index, string-dictionary/const/boolean compression, `Spiller`; **group-commit** batch flushing (Fase 7); **vector index restore** from persisted catalog (`restore_vector_index` / `create_vector_index_with_id`) with HNSW graph rebuild at startup |
| Transactions | MVCC snapshot isolation (readers never block) + optional **OCC multi-writer** (`SET concurrent_writes`): row-level write set validated at commit, losers roll back via undo records; two-phase commit gate (`AtomicBool`+`Condvar`); auto-checkpoint worker |
| Extensibility | Three registries: `FunctionRegistry` (259 fns), `ExtensionRegistry` (`Extension` trait), `StandaloneCallRegistry` (`CALL` handlers). 15 feature-gated extension crates via `register_builtin_extensions`; GDS reaches graphs through `CatalogGraphSource` (dependency inversion) |
| Memory governance | `memory_accountant`/governor in `akar-common` budgets buffers (spill vs. retain) across agg/join/storage; Extend column pruning and cross-product row caps (F6) complement the governor for full-scan blow-up |
| Layering | `akar-common → (storage, txn, catalog, parser) → binder → planner → optimizer → processor → graph → akar-main → (cli, server, c, wasm, python)`. `akar-storage` depends only on `akar-common` |
| Crate graph | 35 workspace members under `akar-core/`, version **0.2.2**; binding crates `akar-c`, `akar-python`, `akar-wasm` are not published to crates.io |
...
| Planning & Optimization | Logical plans (59 ops), 25-pass rewrite, join reorder, EXPLAIN | `akar-core/akar-planner`, `akar-core/akar-optimizer` |
...
| Public API & Shells | `Database`/`Connection`/`QueryResult`/`PreparedStatement`; CLI REPL, TCP server, C FFI, WASM, Python (`akar`, Kuzu drop-in), migrate | `akar-core/akar-main`, `akar-cli`, `akar-server`, `akar-c`, `akar-wasm`, `akar-python`, `akar-migrate` |
| Common Infrastructure | 37 logical/19 physical types, `Value`, `DataChunk`, `ArrowVector`, unified errors, `TaskSystem`, memory accountant, VFS + `Extension` framework | `akar-core/akar-common`, `akar-extension` |

## Core Flows

1. **Query execution** — `Connection::query` parses → binds against `Arc<Mutex<Catalog>>` → plans → optimizes (25 passes; plan-cache hit skips parse→optimize, LRU, invalidated by catalog version) → captures MVCC `(snapshot_ts, commit_history)` → `QueryProcessor` executes operator tree over `DataChunk`s → `QueryResult`. Extend column pruning collects downstream-tail references at top level (disabled for child sub-plans); `forward_limit_budget` pushes LIMIT+SKIP down to `PhysicalCrossProduct`. DDL writes `catalog.json` and signals checkpoint; writes re-wrap then refresh vector indexes.
...
6. **Dream consolidation** — `DreamOrchestrator::run_cycle` runs NREM → SUPERSEDES → REM → Insight → AFE → Synthesis → DAE over the graph DB; server session drives `dream_control` (status/pause/resume) with `GraceBackend` (stub) or `GraphBackend` (Cypher-executed).
...
| Version gates | `STORAGE_VERSION` bump without auto-migration is breaking → SemVer `0.2.0`; optimizer pass count pinned at 25 |
...
| Optimizer | `akar-core/akar-optimizer/src/` | `optimizer.rs`, `passes/flat/`, `passes/tree/` (incl. `vector_similarity.rs`, `art_range_scan.rs`, `fts_predicate_pushdown.rs`, `cardinality.rs`); `fts_estimate.rs` |
| Execution core | `akar-core/akar-processor/src/` | `processor/mod.rs` (`QueryProcessor`, `forward_limit_budget`), `processor/mapper/`, `physical/` (incl. `PhysicalCrossProduct` row caps, `PhysicalExtend` column pruning, `fts_sync.rs` FTS commit sync), `expression_evaluator.rs`, `processor/extend_prune.rs` (`ExtendPrune`, `collect_extend_prune`, `keep_column`) |
...
| Catalog | `akar-core/akar-catalog/src/lib.rs` | `Catalog` + entries; persisted `catalog.json` |
| Search/FTS/Vector | `akar-core/akar-search/`, `akar-fts/`, `akar-vector/` | `native_bm25.rs`, `hybrid.rs`/`hybrid_scan.rs`/`rrf.rs`/`multi.rs`/`fused.rs`; Tantivy index lifecycle; HNSW (ahash visited/node maps, quickselect top-k) + SIMD single-pass distance kernels |
| Graph/GDS | `akar-core/akar-graph/`, `akar-core/akar-algo/` | `graph.rs`, `gds/`, `src/lib.rs` (18 algos) |
| Dream/ML | `akar-core/akar-dream/`, `akar-core/akar-ml/` | `orchestrator.rs`, `phases/`, `embed.rs`, `lstm.rs` (multi-layer `LstmModel<F>` f32/f64; batch `train` + online `train_pair` BPTT + `forward_sequence_hidden` for variable-length sequence hidden-state extraction; `save_bin`/`load_bin` binary persistence; PyO3 `akar.lstm`) |